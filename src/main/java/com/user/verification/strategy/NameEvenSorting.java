...
public class NameEvenSorting implements SortingStrategy {
...
//                .collect(Collectors.toList());
    	...
    	return users.stream().sorted((u1, u2) -> Integer.compare(u1.getName().length() % 2, u2.getName().length() % 2)).collect(Collectors.toList());
    }
}
